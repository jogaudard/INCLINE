...
source("https://raw.githubusercontent.com/jogaudard/common/cleaning_winter/fun-fluxes.R")
...
measurement <- 300 #the length of the measurement taken on the field in seconds
...
winter <- read_csv("data/C-Flux/winter_2021/winter_field-record_2021.csv", na = c(""), col_types = "fffccfc") %>% 
...
co2_conc_winter <- match.flux(fluxes,winter)

#adjusting the time window
# import cutting
cutting <- read_csv("data/C-Flux/winter_2021/winter_cutting_2021.csv", na = "", col_types = "dtt")

co2_conc_winter_cut <- co2_conc_winter %>% 
...
co2_conc_winter_cut <- co2_conc_winter_cut %>% mutate(
...
ggplot(co2_conc_winter_cut, aes(x = datetime, y = CO2, color = cut)) +
  geom_line(size = 0.2, aes(group = ID)) +
  scale_x_datetime(date_breaks = "1 min", minor_breaks = "10 sec", date_labels = "%e/%m \n %H:%M") +
  # scale_x_date(date_labels = "%H:%M:%S") +
  facet_wrap(vars(ID), ncol = 36, scales = "free") +
  ggsave("winter_detail.png", height = 60, width = 126, units = "cm")
