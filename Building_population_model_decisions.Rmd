...
Because all attributes had a significant impact on biomass, we choose coefficients from all attributes and because siteID was not significantly different we use the same same intercepts for each site from the model.
...
Hence, we choose the predicted number of seeds per capsule from the model: `r Seeds_per_capsule_SP`
...
Hence the number of seeds per capsule for Veronica alpina did not depend on size, site or number of capsules in the mother. We used the constant from the null model in the population model:  `r as.numeric(exp(fixef(seed_VA_null)))`
...
Seedling establishment in cold and non-vegetated plots: `r seedling_est_VA_C_NoVeg`
Seedling establishment in cold and vegetated plots: `r seedling_est_VA_C_Veg`
Seedling establishment in warm and non-vegetated plots: `r seedling_est_VA_OTC_NoVeg`
Seedling establishment in warm and vegetated plots: `r seedling_est_VA_OTC_Veg`
...
Seedling establishment in non-vegetated plots: `r seedling_est_SP_NoVeg`
Seedling establishment in vegetated plots: `r seedling_est_SP_Veg`

##Seedling size information  

###Sibbaldia procumbens  

We investigated whether seedlings size changed between treatments (control, extant trait transplants, novel trait transplants, removal), and warmed and unwarmed plots. Using nested site/block/plot as random effect, and a backward selection method to eliminated variables. Neither warming, nor the treatments affected seedling size. However, when we combined all the vegetated treatments (control, extant traits transplant and novel traits transplants), and compared that with the non-vegetated treatments (removal), we saw a difference: 
...
We also calculated maximum seedling size by adding two standard deviations from the mean of the vegetated seedling size (because it was the largest size), as a cut off when assigning parents of the clonal offsprings, which was: `r SP_max_seedling_size`
